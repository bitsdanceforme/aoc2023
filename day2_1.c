...
int parseLine(char* inputString, int lineNumber)
{
    struct tuple tup;
    int gamePossible = 0;
    ...
    { 
        ...
        // parse tuple
        if ( (tup.red <= WINRED) && (tup.green <= WINGREEN) && (tup.blue <= WINBLUE))
        {
        ...
        {
            gamePossible = 0;
            break; // this game isn't possible
        }
    }
    return gamePossible;
}
...
{
    char testLine[] = "Game 1: 8 red, 4 green, 7 blue; 2 blue, 4 red;";
    ...
    while(1)
    {
        ...
        gamePossible = parseLine(inputString, gameNumber);
        printf(" is %s possible\n", gamePossible == 1? "": "not");

        if(gamePossible)
        {
            totalValue += gameNumber;
        }
        // game 3, 5, 6, 10 should be skipped
        gameNumber++;
    }
    printf("totalValue: %d\n", totalValue);
